...
| --- | --- | --- | --- |
| R008-0.1 | Composition strategy: this template = governance envelope, AI-DLC = construction-phase engine | deferred | `prompt-075`: "AI-DLC positioning relative to `awslabs/aidlc-workflows` ... deliberately out of scope here — needs its own scoping decision" |
...
| R008-0.6 | Root `LICENSE` (MIT-0) | done | `LICENSE` at root; `prompt-066` |

### Seção N — Nomenclatura de Arquivos

| ID | Proposal | Status | Decision / Evidence |
| --- | --- | --- | --- |
| R008-N.1 | Rename 6 `agents/*-prompt.md` files (drop `-prompt` suffix) | done | `agents/orchestrator.md`, `phase-reviewer.md`, `adversarial.md`, `doc-consistency.md`, `init.md`, `tool-discovery.md`; `prompt-065` |
| R008-N.2 | New `agents/fast-track.md` | rejected | `prompt-067`: collides with the existing fast-track roadmap path (`roadmap.md` §4.3) |
...
| ID | Proposal | Status | Decision / Evidence |
| --- | --- | --- | --- |
| R008-E.1 | `docs/adr/`, `docs/strategy/`, `docs/visuals/` split out of `docs/manuals/` | done | Live dirs confirmed; `docs/adr/0004-docs-category-directories.md`; `prompt-064` |
| R008-E.2 | `docs/manuals/phases/` (fragment `operation-manual.md`) | rejected | `prompt-064`: "`operation-manual.md` itself stays a single, unfragmented file, out of scope per ADR-0003's still-valid Alternative #2 reasoning" |
...
| R008-E.4 | `adapters/` directory (move `CLAUDE.md`/`AGENTS.md`, root symlinks) | rejected | Batch-2 decision (2026-07-24): Claude Code requires `CLAUDE.md` literally at root (the report's own text admits this). Symlink indirection adds fragility for a cosmetic reorg; `.github/scripts/check-adapter-sync.js` (`prompt-035`) already keeps `CLAUDE.md`/`AGENTS.md` in sync without a folder move. |
...
| ID | Proposal | Status | Decision / Evidence |
| --- | --- | --- | --- |
| R008-T.1 | Fragment `operation-manual.md` by phase | rejected | Same ADR-0003 citation as R008-E.2 |
| R008-T.2 | Split orchestrator into system/tasks files | rejected | Batch-5 decision (2026-07-24): doesn't map onto how `orchestrator.md` is actually organized (by cross-cutting Step/Rule, not by phase) — genuinely per-phase content already lives in `roadmap.md` and is referenced, not inlined. The real size concern stays tracked under R008-2.7 instead. |
...
| R008-T.5 | `STATE.md` reformatted as narrative "SESSION CONTEXT" (phase/mode/bolt/blockers/next action) | done-scoped | `docs/STATE.md` exists and is the mandated first read, but is a generated table of every living document's title/type/status/version (`prompt-034`, predates the report), not the proposed narrative session-context block |

### Section 1 — Redução de Overhead de Governança

| ID | Proposal | Status | Decision / Evidence |
| --- | --- | --- | --- |
| R008-1.1 | Fast-track as official mode, calibrated by AI-DLC "Bolt" | done-scoped (full/short/fast-track) / rejected (Bolt calibration) | `roadmap.md` §4.3 has fast-track; `prompt-067` explicitly rejects the Bolt vocabulary/collision |
...
| R008-1.3 | `STATE.md` as SESSION CONTEXT bootstrap + CI auto-regen trigger + `CLAUDE.md` read-first instruction | done-scoped | Read-first instruction lives in `operation-manual.md`/orchestrator, not literally `CLAUDE.md`; CI **checks** staleness (fails, doesn't auto-write); predates report (`prompt-034`) |
...
| R008-2.3 | CI auto-regeneration of `STATE.md` | done-scoped | `state-staleness-check.yml` fails on staleness rather than auto-writing; predates report (`prompt-034`) |
...
| R008-2.5 | `scripts/bootstrap.js` interactive CLI | rejected | Batch-1 decision (2026-07-24): `/template-init` already solves this — conversational, context-aware, idempotent (`prompt-039`). A parallel static CLI script would duplicate it and risk drift between the two mechanisms. |
...
| R008-2.9 | Extend `step-reference-check.yml` to validate phase/artifact consistency | rejected (as proposed) / resolved via R008-2.1 | Batch-1 decision (2026-07-24): wrong tool — that checker validates this template's own `operation-manual.md` step cross-references, not a generated project's runtime phase state. The real underlying need rode on R008-2.1, resolved 2026-07-27 by `docs/references/gate-verification-template/` (`prompt-083`). |
| R008-2.10 | Consolidate 7 workflows into `governance.yml` | deferred | `prompt-075`: "consolidating the repo's 7 separate `.github/workflows/*.yml` files ... deliberately out of scope here" |
| R008-2.11 | `scripts/agent-evaluator/` (AIDLC Evaluator pattern) | deferred | Batch-1 decision (2026-07-24): valuable in principle — the testing dimensions are already documented (`agent-design-guide.md`, `prompt-056`) — but real golden-testing for prompt-based agents means invoking an LLM in CI and grading non-deterministic output, a real new cost/infra commitment, not a batchable fix. Revisit if agent-prompt regressions actually start happening, or on explicit request to invest in agent-eval infra. |
...
| ID | Proposal | Status | Decision / Evidence |
| --- | --- | --- | --- |
| R008-3.1 | Replace README Mermaid duplicate with a link to the visuals overview | done | `README.md`'s document map is a pointer, not a copy; `prompt-052`/`064` |
| R008-3.2 | Split orchestrator system/task files, control instruction budget | rejected | Same as R008-T.2 |
| R008-3.3 | CODEOWNERS protecting `docs/manuals/`, `docs/adr/`, `agents/` | done | `.github/CODEOWNERS`; `prompt-075` |
...
| R008-3.6 | Consolidate ADRs into `docs/adr/` with short names | done | Confirmed live; `prompt-064`/`072` |
| R008-3.7 | Updated Agent Catalog table in `agent-design-guide.md` | done-scoped | Equivalent table exists in `operation-manual.md`'s Document map instead, predating the report (`prompt-044`) |
| R008-3.8 | Semantic-versioned frontmatter with a `changelog` field per agent | done-scoped (pre-existing) | `version` field present on all `agents/*.md`; body-changelog convention (`prompt-033`, predates report) substitutes for a frontmatter `changelog:` field |
| R008-3.9 | Move `go-to-market.md`, roadmap to `docs/strategy/` | done | `docs/strategy/roadmap.md`, `go-to-market.md`; `prompt-064`/`072` |
...
| R008-3.13 | Document `*.opt-in.md` extension pattern | done | Batch-6 decision (2026-07-24): cheap (one paragraph, no mechanism built), genuinely rounds out `agent-design-guide.md`'s existing workflow-vs-agent spectrum (`prompt-056`) with an even-lighter tier. Documented as an available pattern only, no actual opt-in extension file built yet (`prompt-081`). |

### Section 4 — Adoção e Clareza para Novos Usuários

| ID | Proposal | Status | Decision / Evidence |
| --- | --- | --- | --- |
| R008-4.1 | `QUICKSTART.md` (max 1 page) | done | `QUICKSTART.md`; `prompt-066` |
| R008-4.2 | Internal `README.md`s in `agents/`, `docs/manuals/`, `docs/adr/`, `docs/strategy/`, `adapters/` | rejected | Batch-4 decision (2026-07-24): duplicates the existing single canonical navigational artifact — `docs/STATE.md` plus `operation-manual.md`'s Document Map (`prompt-044`) already list every doc/agent/path/purpose in one place. Four more READMEs would duplicate and drift from it, against ADR-0003's pointer-over-copy principle. |
...
| R008-4.6 | "First session in 30 seconds" flow in QUICKSTART | done | `/template-init` → `/orchestrator` flow; `prompt-066` |
| R008-4.7 | Session sequence diagram | done-scoped | `docs/visuals/template-visual-overview.md`'s phase-execution sequence diagram is conceptually adjacent, not the literal session-bootstrap sequence proposed; predates report (`prompt-052`) |
...
| R008-12.2 | Bolt | rejected | `prompt-067`: naming collision with the existing fast-track roadmap path |
...
| R008-12.4 | Reverse the Conversation Direction (as AI-DLC-named practice) | done | Batch-6 decision (2026-07-24): this is already how the orchestrator behaves (Step 0/1/2 proactively drive with proposals and confirmation questions); named explicitly in `agents/orchestrator.md` (`prompt-081`), judged on merit with no external dependency, per the human's explicit direction. Related to R008-1.6, now resolved together. |
...
| R008-12.6 | PRFAQ template | done | Batch-6 decision (2026-07-24): not AI-DLC-specific IP — an established market-standard technique (Amazon's "working backwards" method). Added as `docs/manuals/examples/spec-prfaq-template.md` (`prompt-081`), optional, matching the existing worked-example pattern. |
...
| R008-12.8 | Deployment Units vocabulary | rejected | Batch-6 decision (2026-07-24): same missing-home problem; Phase 7 already has concrete deployment vocabulary/artifacts (`deploy/` templates, IaC-first, staging→production gate — `prompt-041`) more actionable than an abstract label. |
...
| R008-15.1 | `business/` subfolder + 4 business schemas (`cost-breakdown.json`, `billing-summary.json`, `pricing-benchmark.json`, `quote-estimate.json`) | deferred | `prompt-068`: "needs ≥3 real sampled projects to be trustworthy, and none exist yet" — explicit revisit condition |
...
| R008-17.1 | `billing_model` field in `STATE.md` SESSION CONTEXT | rejected | Batch-7 decision (2026-07-24): depends on two things already decided against — `STATE.md` staying a narrative "SESSION CONTEXT" block (it's a generated table instead, `prompt-034`) and the business/billing layer existing (deferred, R008-15.1). No home for this field under either standing decision. |
...
| R008-14.4 | `.github/workflows/telemetry-refresh.yml` | rejected | Batch-7 decision (2026-07-24): no aggregation/computation logic exists to refresh — the adopted ledger is a simple per-session JSONL append (`prompt-068`), not something with periodic recomputation. The scripts this workflow would run (R008-14.3) were already rejected. |
...
| R008-20.3 | Native token-count API vs. char-estimate investigation | done | `prompt-068`/`CHANGELOG.md`: checked whether `rtk` could source it — documented finding ("it can't yet"), schema gained a `source: "rtk" \| "estimate"` field |
