...
2. Configure `exempt.self_qualifying` to recognize the phrase already written into the ~11 sites hand-qualified in `003-close-restart-followon-drift.md` ("archived private-repo sequence"), so that existing, deliberate qualification is respected rather than re-flagged.
