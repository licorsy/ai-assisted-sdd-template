...
| `004-adopt-dead-citations-shadow-rule` | active | Enables `docs-governance` v1.2.0's `dead_citations` rule as a non-blocking shadow rule, replacing the per-round LLM sweep for dangling `prompt-NNN` citations with a mechanical check. |
