...
| `013-release-readiness-batch` | archived | Named batch of four fixes that are only correct before `v1.2.0` is tagged: folds `[Unreleased]` into the release section, merges its duplicate `### Changed` groups, adds the missing `release-integrity` row to the Step 15 table, and clears six prompts left at `active` after their work merged. |
...
| `015-rename-generic-test-job-context` | active | Renames `governance-scripts-tests.yml`'s job id from the generic `test` to `governance-scripts-tests`, closing the silent-collision risk `014` recorded and deliberately declined, and amends the comment block shared by all six governance workflows to ask for a name specific to what the job checks rather than merely distinct. The ruleset swap that must follow is a separate, human-permissioned step. |
