...
- v3.51: `docs/prompts/` restarted inside this repository (numbering fresh at `001`; the pre-2026-07-31 archive stays in the archived private source repo and is not migrated, `docs/prompts/001-restart-prompt-archive-and-source-of-truth.md`). Step 11's dangling `prompt-004`/`prompt-005` citation dropped; Step 12 rule 1 records this repository as the current source of truth; rule 3's worked-example citation repointed at `001-restart-prompt-archive-and-source-of-truth.md`.
...
| Living memory | Stores decisions, risks, status, ADRs, and versioned instructions | `agents/`, `.specify/`, `docs/manuals/`, `docs/adr/`, `/docs`, `/CHANGELOG.md` |
...
| Enforcing the Step 9 documentation frontmatter rule | `.github/workflows/docs-frontmatter-lint.yml` (prompt-013) | Automates the structural/enum checks in `documentation-metadata-standard.md` Section 9 instead of relying on manual review alone |
| Enforcing that internal Markdown links resolve | `.github/scripts/check-internal-links.js` + `.github/workflows/broken-link-check.yml` (prompt-017) | Makes `documentation-metadata-standard.md` Section 9's check 4 ("no dangling paths") mechanical for link-formatted references, catching renames and moves at PR time |
| Enforcing the newest-3 body-changelog retention cap | `.github/scripts/check-changelog-retention.js` + `.github/workflows/changelog-retention-check.yml` (prompt-055) | Makes `documentation-metadata-standard.md` Section 2.1's retention rule mechanical rather than a "remember to" rule |
| Keeping the frontmatter-lint workflow's and CODEOWNERS's living-doc scope in sync with the canonical list | `.github/scripts/check-scope-consistency.js` + `.github/workflows/scope-consistency-check.yml` (prompt-088) | Makes "which directories are living documents" single-sourced in `.github/scripts/doc-scope.js`, so the two files that can't literally import it (a workflow's YAML, CODEOWNERS) can't silently drift from it either |
...
| Answering "where are we?" without a directory sweep | Generated `docs/STATE.md` (`.github/scripts/generate-state.js`, staleness-checked by `.github/workflows/state-staleness-check.yml`, prompt-034) | One deterministic consolidated read of every living document's status/version/freshness; excludes the historical `docs/prompts/` archive |
