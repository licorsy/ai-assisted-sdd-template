...
- v3.61: Step 9's frontmatter obligation gains the changelog/version-header clause and a `.specify/` best-effort qualifier (both adapters had drifted from this file, restating the rule without them — a cross-repo documentation audit found the gap); reconciled against `documentation-metadata-standard.md` Section 2.1's new mechanical-only exemption so the two documents no longer state opposite rules. Step 15's `generate-state.js` and `check-scope-consistency.js` rows corrected to match what those scripts actually do today; a row added for the (retired) `check-public-sanitization.js`.
- v3.60: Step 12 rule 3's archived-repo citation reworded to the established qualifier phrase ("archived private-repo sequence, not a citable path in this repository"), matching the ~72 sites already using it, as part of closing out the `dead_citations` backlog.
- v3.57: Document map's "Reference material" row gains `docs/references/missing-data-vocabulary.md`, which the same edit that added the `Glossary` row had left out; the honest-degraded-states pointer stops carrying its own count, which contradicted three other statements of it, and defers to the reference that owns the list; the runtime-trigger prose now says "a non-matching change", matching the flowchart node it mirrors (`fix-verifier` pass).
...
| Go-to-Market Roadmap | Optional, parallel roadmap for the commercial lifecycle (positioning, pricing, launch, marketing, sales, retention, business retrospective) - not a gate every release cycle passes through | `docs/strategy/go-to-market.md` |
...
| Glossary | This model's vocabulary in market-standard terms - release cycle vs. Scrum Increment, the phase model named as Stage-Gate with each stage's agile equivalent, and the four delivery surfaces mapped onto real artifacts | `docs/manuals/glossary.md` |
...
| Reference material | Token-economy decisions, an unvetted tools-ecosystem shortlist, the missing-data vocabulary, ready-to-apply GitHub-hardening/deployment templates, a session-telemetry convention, and an opt-in phase-gate artifact check, for generated projects | `docs/references/` |
...
5. **Phase-exit brief format** - today's free-prose end-of-phase report, or a standardized six-field brief - opt-in only, defaulting to free-prose when unstated (`prompt-082` (archived private-repo sequence, not a citable path in this repository)).
...
- Every Markdown documentation file in this repository — including `agents/` and every category directory under `docs/` enumerated in `docs/manuals/documentation-metadata-standard.md` Section 1, plus any artifact the roadmap generates elsewhere under `.specify/` (best-effort there: Spec Kit tooling may regenerate those files and strip custom frontmatter, so apply it but don't treat its survival as guaranteed) — must carry the YAML frontmatter schema defined in `docs/manuals/documentation-metadata-standard.md`; apply or update it whenever the file is created or materially changed, alongside the version header and — for a guidance change, not a mechanical-only edit (see Section 2.1's distinction) — the changelog.
...
- Render an absent, degraded, or withheld value with the shared vocabulary in `docs/references/missing-data-vocabulary.md` rather than inventing a token per surface: `[x]` not available, `[z]` not applicable, never the ambiguous `NA`, legend above the table, and never a line joined across a gap. The obligation to be honest about gaps was already stated in several places here before the vocabulary existed, each with its own token; that reference owns the list.
...
`docs/manuals/documentation-metadata-standard.md` Section 4.1 defines the `status` field's shape for `docs/prompts/` files; this step states the operational trigger points for changing it:
...
4. **deprecated** - flip to `status: deprecated` when a drafted prompt is declined or superseded before execution; record the decline reason in the prompt's own body, not only in `docs/prompts/PROMPT-INDEX.md`.
...
| Keeping CODEOWNERS' protected paths in sync with `doc-scope.js`'s `PROCESS_DIRS` | `.github/scripts/check-scope-consistency.js` + `.github/workflows/scope-consistency-check.yml` (prompt-088) | Makes "which directories are living documents" single-sourced in `.github/scripts/doc-scope.js`; `.docgov.config.js` now `require()`s `CATEGORY_DIRS`/`SCOPE_FILES` directly (not `PROCESS_DIRS`, which only CODEOWNERS uses), so CODEOWNERS — the one file that can't literally import anything — is the only remaining target this check needs to compare |
...
| Answering "where are we?" without a directory sweep | Generated `docs/STATE.md` (`.github/scripts/generate-state.js`, staleness-checked by `.github/workflows/state-staleness-check.yml`, prompt-034) | One deterministic consolidated read of every `CATEGORY_DIRS` document's status/version/freshness, sourced from `doc-scope.js`'s `ROOT_FILES` for root files; excludes `docs/prompts/` regardless of status (kept lean - `docs/prompts/PROMPT-INDEX.md` is that directory's own status index) and excludes `README.md`/`CLAUDE.md`/`AGENTS.md` (not in `ROOT_FILES`, which is a narrower, generator-specific subset of the full root-entry-point set — see `doc-scope.js`'s own header comment) |
...
| Flagging secret/PII-shaped content before syncing to a public mirror | `.github/scripts/check-public-sanitization.js` (prompt-108) — **retired**, along with its only caller | This repository is now the sole public source of truth (`docs/adr/0005-public-release.md`); kept only as historical record |
