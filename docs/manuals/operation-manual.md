...
- v3.57: Document map's "Reference material" row gains `missing-data-vocabulary.md`, which the same edit that added the `Glossary` row had left out; the honest-degraded-states pointer stops carrying its own count, which contradicted three other statements of it, and defers to the reference that owns the list; the runtime-trigger prose now says "a non-matching change", matching the flowchart node it mirrors (`fix-verifier` pass).
